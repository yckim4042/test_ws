...
// 포인트를 구의 표면으로 사영하는 함수
pcl::PointCloud<pcl::PointXYZ>::Ptr projectToSphere(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    ...
    for (const auto& point : cloud->points) {
        ...
        if (distance > 0) {
            ...
            projected_point.x = point.x / distance; // x 좌표를 정규화
            projected_point.y = point.y / distance; // y 좌표를 정규화
            projected_point.z = point.z / distance; // z 좌표를 정규화
            projected_cloud->points.push_back(projected_point); // 사영된 점 추가
        }
    }
...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& input) {
    ...
    //1. 바운더리 찾는 함수
    //1-1. 구면 사영 
    pcl::PointCloud<pcl::PointXYZ>::Ptr pointclound_for_lines;
    pointclound_for_lines = projectToSphere(cloud);
    pcl::io::savePLYFile("/home/rp/urop/test_ws/src/boundary_evaluation/boundary_gt/boundary.ply", *pointclound_for_lines);

    //1-2. 구면 사영된 점들을 기준으로 line을 구함
