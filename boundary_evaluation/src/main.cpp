...
std::vector<int> ransacCircleClustering(const pcl::PointCloud<pcl::PointXYZ>::Ptr &points, const Eigen::Vector4f &plane_coeffs,
                                        ...
                                        float radius_threshold , int iterations ) {
    ...
    for (int cluster_count = 0; cluster_count < 12; ++cluster_count) {
        int max_inliers = 0;
...
std::vector<Eigen::Vector3f> analyzeAndFitCirclesWithRansacClustering(const pcl::PointCloud<pcl::PointXYZ>::Ptr &noise_free_points,
                                                                      const Eigen::Vector4f &plane_coeffs,
                                                                      float min_radius = 0.02, float max_radius = 0.04,
                                                                      float radius_threshold = 0.005, int iterations = 200) {
    ...
    for (int label : unique_labels) {
        ...
        if (cluster_points->points.size() >= 5) {
            ...
            if (final_radius >= 0.04 || final_radius <= 0.024) {
...
// 원 중심 정렬 함수
std::vector<Eigen::Vector3f> sortRectanglePoints(const std::vector<Eigen::Vector3f> &points) {
    std::vector<Eigen::Vector3f> sorted_points = points;

    // 우선 y 좌표를 기준으로 내림차순 정렬합니다.
    std::sort(sorted_points.begin(), sorted_points.end(), [](const Eigen::Vector3f &a, const Eigen::Vector3f &b) {
        return a.y() > b.y();
    });

    // y 좌표별로 x 좌표를 기준으로 정렬합니다.
    for (int i = 0; i < 12; i += 4) {
        std::sort(sorted_points.begin() + i, sorted_points.begin() + i + 4, [](const Eigen::Vector3f &a, const Eigen::Vector3f &b) {
            return a.x() < b.x();
        });
    }
...
Eigen::MatrixXf board_points(3, 12);
...
struct Residuals {
    ...
    bool operator()(const T* const theta, const T* const x_offset, const T* const y_offset, T* residual) const {
        ...
        // 12개의 점을 처리할 수 있도록 행렬 크기를 변경
        Eigen::Matrix<T, 3, 12> transformed_points = R_board.template cast<T>() * board_points.template cast<T>();
        Eigen::Matrix<T, 2, 12> transformed_points_2d = transformed_points.topRows(2);

        Eigen::Matrix<T, 2, 12> transformed_world_points_2d = transformed_world_points.topRows(2).template cast<T>();

        Eigen::Matrix<T, 2, 12> diff = transformed_points_2d - transformed_world_points_2d;
...
std::tuple<Eigen::Matrix4f, float, float, float, float, float, float> optimize_rotation_transform(
    ...
    Eigen::Vector3f center_9 = cluster_centers[8];
    Eigen::Vector3f center_10 = cluster_centers[9];
    ...
    Eigen::Vector3f w = center_10 - center_9;
    w.normalize();  // 벡터 정규화

    Eigen::Vector3f v = Eigen::Vector3f(a, b, c).cross(w);
    Eigen::Matrix4f R = compute_R(w, v, a, b, c, center_9);
    //std::cout << R<<std::endl;
    ...
    float pitch = std::asin(-final_transform(2, 0));
    float roll = std::atan2(final_transform(2, 1), final_transform(2, 2));

    return std::make_tuple(final_transform, roll, pitch, yaw, final_transform(0, 3), final_transform(1, 3), final_transform(2, 3));
}

int main(int argc, char **argv) {
    board_points << 0.09f, 0.18f, 0.27f, 0.36f, 0.09f, 0.18f, 0.27f, 0.36f, 0.09f, 0.18f, 0.27f, 0.36f,
                0.27f, 0.27f, 0.27f, 0.27f, 0.18f, 0.18f, 0.18f, 0.18f, 0.09f, 0.09f, 0.09f, 0.09f,
                1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f;  // 동차 좌표계로 확장
    // Point cloud 로드
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
    if (pcl::io::loadPLYFile<pcl::PointXYZ>("/home/rp/urop/test_ws/src/boundary_evaluation/src/boundary_gt/ours_1.ply", *cloud) == -1) {
    ...
    //pcl::io::savePLYFile("/home/rp/urop/test_ws/src/boundary_evaluation/src/boundary_gt/accumulated_pointcloud_3.ply", *noise_free_points);
    ...
    if (!circle_centers.empty()) {
        ...
        std::vector<cv::Point2f> source, dist;
        std::vector<std::pair<cv::Point2f, float>> source_with_x;  // 원래의 (y, z) 좌표와 대응되는 x 값을 저장
        // 변환: Eigen::Vector3f -> cv::Point2f (2D 정보만 사용) + 원래의 x 값을 저장
        for (const auto& center : circle_centers) {
            source.emplace_back(cv::Point2f(center.y(), center.z()));  // X축 정보는 무시하고 2D 정보만 사용
            source_with_x.emplace_back(std::make_pair(cv::Point2f(center.y(), center.z()), center.x()));  // (y, z)와 x 저장
        }
        dist.resize(12);
        // CircleGridFinder 실행
        CircleGridFinder gridfinder(isAsymmetricGrid);
        gridfinder.findGrid(source, cv::Size(4, 3), dist);
        // 변환: cv::Point2f -> Eigen::Vector3f (sorted_centers에서 원래의 x 값을 복원)
        std::vector<Eigen::Vector3f> sorted_centers;
        for (const auto& point : dist) {
            // dist의 각 point에 대응하는 원래의 source에서의 x 값을 찾기
            auto it = std::find_if(source_with_x.begin(), source_with_x.end(), [&point](const std::pair<cv::Point2f, float>& p) {
                return p.first == point;  // y, z가 같은 것을 찾음
            });
            // 원래의 x 값을 사용하여 sorted_centers에 추가
            if (it != source_with_x.end()) {
                sorted_centers.emplace_back(Eigen::Vector3f(it->second, point.x, point.y));  // it->second는 원래의 x 값
            }
        }
