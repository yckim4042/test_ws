...
pcl::PointCloud<pcl::PointXYZ>::Ptr removeWalls(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud,
                                                float x_min = 0.5, float x_max = 2.2,
...
std::vector<pcl::PointIndices> clusterPoints(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, 
                                             float eps = 0.03, int min_samples = 30) {
...
pcl::PointCloud<pcl::PointXYZ>::Ptr findBoundaryPointsImproved(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud,
    ...
    const double tan_cone_angle = std::tan(cone_angle_deg * M_PI / 180.0);  // Precompute tan(cone_angle)

    for (size_t i = 0; i < cloud->points.size(); ++i) {
        const auto &point = cloud->points[i];
        
        // Calculate the distance from the point to the origin (or another point)
        double distance = std::sqrt(point.x * point.x + point.y * point.y + point.z * point.z);
        double radius = distance * tan_cone_angle;
  
        std::vector<int> point_idx_radius_search;
        std::vector<float> point_radius_squared_distance;
        
        // Perform a radius search to get potential neighbors within the radius
        if (kdtree.radiusSearch(cloud->points[i], radius, point_idx_radius_search, point_radius_squared_distance) > 0) {
            ...
            for (const auto &neighbor_idx : point_idx_radius_search) {
                ...
                // Calculate vector from the point to the neighbor
                Eigen::Vector3f neighbor_vec(neighbor.x - point.x, neighbor.y - point.y, neighbor.z - point.z);
                Eigen::Vector3f neighbor_conic(neighbor.x, neighbor.y, neighbor.z);
                // Filter out neighbors that are too close
                if (neighbor_vec.norm() > min_distance) {
                    // Calculate the angle between the vector and the origin to point vector
                    Eigen::Vector3f point_vec(point.x, point.y, point.z);
                    double angle = std::acos(point_vec.normalized().dot(neighbor_conic.normalized()));
                    
                    // Check if the angle is within the cone angle
                    if (angle <= cone_angle_deg * M_PI / 180.0) {
            ...
            // Calculate centroid of valid neighbors
            ...
            // Calculate mean distance and centroid distance
            ...
            // Check if direction variance is below the threshold
...
std::cout<<2;
    ...
    savePoseToFile(roll, pitch, yaw, x, y, z, "/home/rp/urop/test_ws/src/boundary_evaluation/src/ours_pose.txt");
    }
